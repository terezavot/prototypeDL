...
#model_folder = os.path.join(os.getcwd(), "saved_model", "mnist_model", "mnist_cae_2")
model_folder = os.path.join(os.getcwd(), "saved_model", "test_model", "test_cae_mnist_1500")
# folder for saving last layer of prototypes one prototype at a time
#prototype_folder = os.path.join(os.getcwd(), "saved_model", "mnist_model", "prototypes")
prototype_folder = os.path.join(os.getcwd(), "saved_model", "test_model", "test_cae_mnist_1500","prototypes")
makedirs(model_folder)
makedirs(prototype_folder)
...
n_classes = 10
### Specify classes that are being predicted in this task so it can be easily visualised in the final prototype matrix ###
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(n_train_batch):
            batch_x, batch_y = mnist.train.next_batch(batch_size) #batch_y size is 250,10
        ...
        if epoch % save_step == 0 or epoch == training_epochs - 1:
            ...
            for i in range(n_rows):
                for j in range(n_cols):
                    if i * n_cols + j < n_prototypes:
                        ...
                        h.imshow(prototype_imgs[i * n_cols + j].reshape(input_height, input_width),
                                       cmap='gray',
                                       interpolation='none')
                        h.axis('off')

                        plt.savefig(os.path.join(prototype_folder, 'prototype_result-' + str(i) + str(j) + '.png'),
                                    transparent=True,
                                    bbox_inches='tight',
                                    pad_inches=0)
                        plt.close()
            ...
            print(mnist.train.images[:examples_to_show].shape)
...
path_to_pics = "/home/tereza/Desktop/thesis/PrototypeDL/saved_model/test_model/test_cae_mnist_1500/prototypes"
reader = tf.train.load_checkpoint('/home/tereza/Desktop/thesis/PrototypeDL/saved_model/test_model/test_cae_mnist_1500/mnist_cae-99')
...
#df.style.background_gradient().to_excel(
#            f"{prototype_folder}/table.xlsx")
df.to_excel(f"{prototype_folder}/table.xlsx")
with open(f'/home/tereza/Desktop/thesis/PrototypeDL/saved_model/test_model/test_cae_mnist_1500/matrix.html', 'w') as fo:
    fo.write(df.to_html(render_links=True,escape=False))

plt.plot(history['train'])
plt.plot(history['valid'])
#plt.plot(self.history['test'])
plt.axhline(y=history['test'][-1], color='g', linestyle='dotted')
plt.title('model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train', 'val', 'test'], loc='upper left')
plt.savefig(f'{model_folder}/accuracies.png')